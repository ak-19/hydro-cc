...
class TestTimeSeriesEqualizer(TC):
    ...
    def test_case_1(self):
        _, data_points = FileHandler(
            'test-data/input-1.json', '').extract_data()
    ...
    def test_case_2(self):
        _, data_points = FileHandler(
            'test-data/input-2.json', '').extract_data()
        timeseries = TimeSeriesEqualizer(data_points).equalize()
        expected_timeseries = [{"timestamp": 1581609600000, "value": 0.2}, {
            "timestamp": 1581611400000, "value": 3.0}, {"timestamp": 1581613200000, "value": 9.0}]
        self.assertEqual(timeseries, expected_timeseries, 'Test case 2 failed')

    # use case 3
    def test_case_3(self):
        _, data_points = FileHandler(
            'test-data/input-3.json', '').extract_data()
        timeseries = TimeSeriesEqualizer(data_points).equalize()
        expected_timeseries = [{"timestamp": 1581609600000, "value": 8.0}, {
    ...
    def test_case_4(self):
        _, data_points = FileHandler(
            'test-data/GranularityOperations_Input.json', '').extract_data()
        timeseries = TimeSeriesEqualizer(data_points).equalize()
